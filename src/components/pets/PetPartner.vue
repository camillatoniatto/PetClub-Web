              ...
              <v-data-table :headers="headers" :items="pets" :search="search">
                <template v-slot:item="row">
                  <tr>
                      ...
                      <td><v-chip dark :color="getColor(row.item.isAlive)" class="align-center my-5 md-5 white-text"></v-chip></td>
...
export default {
  ...
  methods:{
    ...
    listarPetPartner(){
        register.getPetsPartner().then(response => {
    ...
    editItem (item) {
        ...
        this.petSelecionado.birthdate = moment(this.petSelecionado.birthdate, 'DD/MM/YYYY').format('yyyy-MM-DD')
        this.dialog = true
    },
    deleteItem (item) {
      this.editedIndex = this.pets.indexOf(item)
