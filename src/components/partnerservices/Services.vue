              ...
              <v-card-text>
                <v-container>
                  <v-row>
                    ...
                    <v-text-field v-model="serviceSelecionado.value" label="Valor" type="number"></v-text-field>
...
export default {
  ...
  data () {
    return {
      ...
      dialogDelete: false,
      admin: true,
      // serviceTypeTxt: ['Hospedagem', 'Passeio', 'Serviço Veterinário', 'Banho e Tosa', 'Outros'],
      headers: [
      {
        text: 'Status',
        align: 'start',
...
methods:{
  ...
  remover(service){
      ...
      if(result){
          register.deleteService(service.id).then(response => {
          this.listar()
          this.errors = {}
